...
export const AGENT_TOOLS = [
  tool('ply_delegate', 'Start a Pleiad-managed task using the explicitly selected backend. Returns taskId immediately; completion is delivered automatically. Separate from native spawn_agent. The child inherits the caller approval strength without exceeding it; a stronger child needs one human approval here.', { backend: { type: 'string', enum: ['claude', 'codex', 'procway', 'antigravity'] }, task: str, context: str, cwd: str, model: str, effort: str }, ['backend', 'task']),
