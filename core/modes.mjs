// 承認モードを「2つの軸」で表す。
//
// バックエンドごとのモード名（claude の acceptEdits、codex の yolo、procway の full-auto …）は
 ...
 *
 * どちらが強いとも言えない組み合わせ（procway の always-ask = 作業ディレクトリ・毎回聞く と
 * auto-readonly = 読むだけ・迷ったら聞く）では、**範囲を保ったまま自律だけ下げる**。
