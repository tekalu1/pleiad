...
画面のモック: [remote-window.html](mockups/remote-window.html)（リモートの窓）、[remote-folder-picker.html](mockups/remote-folder-picker.html)（作業フォルダーの「手元から送る」）、[remote-mobile.html](mockups/remote-mobile.html)（モバイルのホスト一覧とホスト名）。
...
2. **帯の色**: ローカルの帯は `--surface-0`（点のグリッド）。リモートの窓は帯を `--fill-primary`、文字と窓のボタンの記号を `--on-fill` にする。
   既存の `paintTitleBar`（`web/client.mjs:1960`）が `.titlebar` の `--bar-end` と `--ink` を読んで `setTitleBar` で送る仕組みをそのまま使い、`:root.remote .titlebar{--bar-end:var(--fill-primary)}` と記号の色の読み先を足すだけ。
   塗りは「1 画面に 1 つ・主要操作だけ」（design-system §2.3）の例外になる。承認モードの YOLO の ⚠ と同じく、取り違えると被害が大きい場所なので例外として認める（§11 未決に代案）
...
- リモートの窓の作業フォルダーの面（入力欄のチップ）の上に 2 つのタブ「**ホストのフォルダー**」（既定。今の面そのもの）/「**手元から送る**」。ローカルの窓・モバイルにはタブを出さない
- 「手元から送る」: 「フォルダーを選ぶ…」（`<input type="file" webkitdirectory>`。手元の OS のダイアログだが、ブラウザーの機能で読むので同じ PC のブリッジは要らない）
...
- 終わったら「送りました · 3,410 件」と、送り先が**その会話の作業フォルダー**になる（未送信の会話はその場で、送信済みの会話は「次のターンから適用」に入る。既存の `nextSettings.cwd`）
- 大きすぎるとき（2 万件超 または 2 GB 超）は送る前に一文で知らせ、除外を見直してもらう（送ることは止めない）
- **リモートの窓にフォルダーをドロップ**したら、その場に「添付する / 作業フォルダーとして送る」を聞く小さな面。ファイルだけのドロップは今までどおり添付（8MB まで）
...
各段で `npm test` を通す（LLM は呼ばない）。中継とトンネルは fake バックエンドのホストで、端末 → 中継 → ホストの往復（会話の送信・承認・ファイルの取得・再接続）を自動で確かめる。
