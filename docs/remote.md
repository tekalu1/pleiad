...
- 認証の失敗は IP ごとに数え（Coolify の前段の後ろなので `X-Forwarded-For` の先頭を使う）、1 分に 10 回で 10 分止める。入場券は 1 回使えば消え、ペアリングの試行はホストごとに 1 分 5 回まで
...
| `RELAY_ENROLL_SECRET` | （必須。無ければ起動しない） | ホストの登録用の秘密。32 バイト以上の乱数 |
...
`tests/relay.mjs`（`npm test` から回す。LLM は使わない）: 登録用の秘密なし・違うトークン・取り消し済みでつながらない、入場券の 1 回きり・失効、ホストが居ないときの閉じ方（4404）、上限を超えるメッセージで切れる、背圧の上限で切れる、ホストの張り直しと `sync` で照合が戻る、2 本目の制御接続で古い方が閉じる。暗号は中継の外なので、試験はバイト列が変わらずに届くことだけを見る。
...
| 2 | #12 | `relay/`（口・照合・上限・ping・状態を持たない）、Dockerfile、`tests/relay.mjs`、Coolify への配置 | — |
