...
10. **帯域**: ~~全イベントを全端末に配る~~ 決定（2026-09-26）: 流れの出来事は各端末が開いている会話の分だけ送る（[ADR 0024](adr/0024-watch-open-session-stream.md)）。`loadSession` の全量・画像の `dataUri` と、中継での圧縮は残っている（2 MB の会話は gzip で 2〜8 分の 1）
