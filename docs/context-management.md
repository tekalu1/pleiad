...
- `${AGENT_HOST_DATA}/context-scans.json`（既定 `~/.agent-host/`）に `version: 3` で保存する。
  ...
  ```
- **追加ルート（探す場所を足す）は種類ごと**（2026-09-27、[ADR 0025](adr/0025-context-global-settings-and-ply-instructions.md)）。足した場所はその種類だけを、その範囲の探す形式（`sources`）で、プロジェクトと同じ置き方（`AGENTS.md`・`.claude/`・`.codex/`・`.agents/skills`・`.mcp.json`）で探す。見つかった行には `root`（足した場所）が付き、画面は「追加した場所」として分ける。既定の追加ルートの行はユーザーの範囲（`scope: 'user'`）、場所の追加ルートの行は作業場所の範囲。
