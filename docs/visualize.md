...
- パネルの部品は `web/side-panel.mjs` の `visualizationSlots` が決める（承認モック `docs/mockups/side-panel-shell.html`）。見出しの横に「可視化」の印、見出しの下は元のパス（作業ディレクトリの中なら相対。元が無ければ「会話に保存された表示」）。ツリーの枠は出さない。下の行は保存した時刻（記録の `at`。今日なら時刻だけ）。再読み込みは出さない（取り直す先が無い）。ホスト上の現在のファイルを読むファイルプレビューとは、同じ面でも読む対象が違う。保存もホストへ取りに行かず、会話に残っている内容を渡す。
